...
const Layout = () => {
  ...
  useEffect(() => {
    ...
    // ✅ ОПТИМІЗОВАНО: Функція тепер викликає легку RPC для підрахунку
    ...
    const channel = supabase
        .channel('admin-layout-listener')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'messages' }, (payload) => {
          // Оновлюємо лічильник при будь-якій зміні в повідомленнях
    ...
    <div style={styles.layout}>
      <aside style={styles.sidebar}>
        ...
        <nav style={styles.nav}>
          <NavLink to="/transfers" style={({ isActive }) => ({ ...styles.navLink, ...(isActive ? styles.activeNavLink : {}) })}><Icon path={icons.transfers} /> Трансфери</NavLink>
          <NavLink to="/my-offers" style={({ isActive }) => ({ ...styles.navLink, ...(isActive ? styles.activeNavLink : {}) })}><Icon path={icons.offers} /> Мої пропозиції</NavLink>
          ...
          <NavLink to="/users" style={({ isActive }) => ({ ...styles.navLink, ...(isActive ? styles.activeNavLink : {}) })}><Icon path={icons.users} /> Користувачі</NavLink>
          <NavLink to="/notifications" style={({ isActive }) => ({ ...styles.navLink, ...(isActive ? styles.activeNavLink : {}) })}><Icon path={icons.notifications} /> Сповіщення</NavLink>
          <NavLink to="/settings" style={({ isActive }) => ({ ...styles.navLink, ...(isActive ? styles.activeNavLink : {}) })}><Icon path={icons.settings} /> Налаштування</NavLink>
        </nav>
...
styleSheet.innerText = `a:hover { background-color: #1f2937; color: #ffffff; } button:hover { background-color: #1f2937; color: #ffffff; }`;
